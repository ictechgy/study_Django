...
urlpatterns = [
    ...
    #myweb안에 views.py파일과 index함수를 만들어보자
    ...
    #blog라는 path패턴으로 접속한 것에 대해 연결시켜 줄 것이 뒤에 있다. -> myweb.views.blog
    #myweb이라는 app 안의 views라는 스크립트. 그리고 그 안의 blog라는 함수
    #include함수를 쓰지 않았기에 정규표현식 마지막에는 $가 들어간다. 무시하고 안쓰고 그러면 어찌 되지?
    #include를 쓰는 것은 받은 url패턴에 대해 다른 app속의 urls.py로 넘겨주고자 할 때 쓰는 것 같다.
    #name 을 쓰는 것은 나중에. name을 쓰면 편리한 기능이 있다고 한다.(역순조회)


    # 정규 표현식 문제
    #1. 1990 ~ 2999 까지의 숫자패턴을 정규 표현식을 사용하여 myWeb의 viewOfYear 함수로 연결하시오.
    url(r'^(199|2[0-9]{2})[0-9]$', 'myweb.views.viewOfYear'),

    #2. 01 ~ 12 까지의 숫자패턴을 정규 표현식을 사용하여 myWeb의 viewOfMonth 함수로 연결하시오.
    ...
    #예) python-django 또는 django-url-pattern 또는 python -> 이런 것을 슬러그(slug)패턴이라고 한다. 뉴스기사에 있어서 url에 제목이 포함될 때 쓰는 패턴이라고 함
    url(r'^([a-z]+-?)+$', 'myweb.views.viewOfPage'),

    

    #선생님 답
    #1. 1990~2999까지의 년도를 정규표현식으로 표현 -> [0-9]{4}라고 할 수도 있겠지만... 1990~1999, 2000 ~ 2999 패턴으로 분리
    url(r'^(199[0-9]|2[0-9]{3})/$', 'myweb.views.viewOfYear'),
    #문제는 이렇게 한 경우 연도값이 함수의 인자로 넘어간다. 혹여나 함수에 매개변수 request만 있다고 한다면
    #사용자의 request를 받지 못하고 매개변수 request로 path연도값이 넘어간다. 따라서 우리는 선택지 두개 중 하나를 선택해야한다.
    #함수에 매개변수 하나를 더 받도록 만들던지, 아니면 여기 패턴에서 인자로 넘기지 않도록 ?:을 추가적으로 써주던지
    # -> 매개변수를 두개 이상 만들면 첫번째에는 무조건 사용자의 http request가 받아지나보다.

    #2. 01~12까지의 숫자를 정규표현식으로 표현 -> 01~09와 10~12를 분리
    url(r'^(?:0[1-9]|1[0-2])/$','myweb.views.viewOfMonth'),

    #3. 1~31까지의 숫자를 정규표현식으로 표현 -> 1~9, 10~29, 30,31 분리
    url(r'^(?:[1-9]|[1-2][0-9]|30|31)/$','myweb.views.viewOfDay'),

    #문제는 이렇게 하는 경우 path에 10~12사이의 숫자를 입력 하면 월에 매치가 된다.
    #이 url패턴은 리스트인데 순서대로 값 검증을 한다. 그러다가 10이라는 path를 만나면 월패턴으로 먼저 인식한다.
    #따라서 구분할 수 있는 구분자 패턴을 추가적으로 넣어줘야 한다.
    #선생님은 그래서 패턴에다가 ^month/ 와 ^day/ 라는 패턴을 추가적으로 넣어주셨다.
    #위와 같이 하면 이제 패턴에 /month/12 와 /day/12로 구분이 되게 된다.

    #4. slug pattern. python, python-django, python-django-web 이런 여러 패턴에 대해 정규표현식으로 표현
    url(r'^(?:[a-z]+-?)+/$','myweb.views.viewOfPage'),
    #마지막에 슬래시 빠지면 패턴인식을 못하네. 즉 사용자가 path에 그냥 /python이라고 쳐도 이는 서버로 갈 때 자동으로 /python/ 이런 형식이 되는 듯 하다.
    #따라서 마지막에 꼭 /를 써줘야 하는 것 같다. 아니면 /? 를 써주든가
    #만약 이 패턴path를 통쨰로 함수로 전달하고자 한다면 r'^((?:[a-z]+-?)+)/$'로 만들어줘야 한다.
    # r'^([a-z]+-?)+/$' 라고 한다면 인자로 여러값이 넘어갈 수도 있을 듯 -> 만약 path로 python-django 가 있다면 'python-' 인자와 'django'인자가 따로 넘어가는 방식이 될 듯 하다. 

    #또 이렇게 slug패턴을 쓰면 admin 패턴이 인식이 안될 수 있다. admin에 접근 안할 것이면 상관 없지만 admin패턴에 접속하고자 한다면
    ...
    #함수에 위치매개변수 두개가 존재한다 가정했을 때 request는 언제나 넘어오겠지만 path에서 인자를 안넘겨주면 오류뜨려나
    #가변인자로 존재한다면 path값 넘겨줘도 되고 안넘겨도 되고. 안넘기면 기본값 쓸테고

    #함수에 인자로서 전달해야 하는 경우라면 전달을 하면 되고 전달할 필요가 없다면 전달하지 않아도 된다.
    # -> path값에 따라 조금 상세하게 함수구동방식과 결과를 세분화하고자 할 때 path값을 넘기면 될 것같다.
