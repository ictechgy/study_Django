...
#그리고 사실 수정이나 삭제에 대해 반환받는 값이 있다면 그것을 이용해서 예기치 못한 오류에 대해 다른 페이지를 연결시켜주는 것도 필요

#HTTP통신이라는게 서버가 어떤 페이지를 넘겨주면 서버는 이제 그 페이지가 뭐였는지 모른다. 따라서 나중에 다시 받았을 때 
#아 내가 어떤 페이지를 보내줬고, 클라이언트가 어떤 페이지를 받았으며 어떤 작업을 했었구나 알 수 있도록 필요한 값들은 미리 다 집어넣어서
#보내놔야 나중에 다시 돌려받았을 때에도 아 이런 작업 하고 있었지 알 수 있다. 
...
#근데 onclick같은거 POST방식으로 넘겨줄 수는 없는건가